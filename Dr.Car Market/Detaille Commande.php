...
try {
  ...
  if(isset($_GET['num']) == true)
  {
     if($_GET["num"] != NULL)
     {
        $Cart = "";
        $Num = 0;
            $Commande = $cn->prepare("SELECT * FROM `ligne_de_commande` WHERE Num_C=?");
            ...
            if($Commande->rowCount() != 0)
            {
              for ($j=0; $j < $Commande->rowCount() ; $j++) 
              {
                ...
                $Num=$Num+1;
                  ...
                  $Prod = $cn->prepare("SELECT * FROM  produit  WHERE  Code_P = ?");
                  $Prod -> execute(array($NumP));
                  if($Prod->rowCount() != 0)
                  {
                        ...
                        $Prix = $line2["Prix_P"];
                        $Cart=$Cart.'
                  <div style="margin-top: 20px;" class="profile-container">
                  <div class="img-container">
                  <img style="" src="'.$Image.'" alt="profile image">
              </div>
              <p style="margin-top: 0px; padding-bottom:25px;" class="info full-name">Produit N°'.$Num.' </p>
                      <div class="nn">
            
                          <p class="cc info place">
                              <span style="font-size: 25px;">📋 </span>'.$Nom.'
...
<body>
    <form id="form1">
     <h1 style="margin-left : -80px" class="tt">Details Commande</h1>
     <section class="bgg footer">
        <div class="share">
            <a href="Mes Commande.php?res=<?php echo $_GET["res"] ?>" class="fab las la-home"> </a>
        </div>
    </section>
        <div class="Contrats">
            <?php echo $Cart; ?>
