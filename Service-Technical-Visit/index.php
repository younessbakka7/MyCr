...
if (array_key_exists('Reserver', $_POST))
    {
        if(isset($_POST["Date"]) && isset($_POST["hours"]))
        {
            if(!empty($_POST["Date"]) && !empty($_POST["hours"]))
            {
                ...
                $RDV = $cn->prepare("SELECT * FROM rdv WHERE Date_R_RV = ? AND Hour = ? AND Type_RV = 'Visit'");
                $RDV -> execute(array($_POST["Date"],$_POST["hours"]));
                if($RDV->rowCount() == 0)
                {
                    $Date = date("Y-m-d");
                    $DES = $_POST["FULLNAME"]." - ".$_POST["NUMBER"];
                    $UTTable = $cn -> prepare("INSERT INTO rdv VALUES(NULL,?,'Visit',?,?,?,?,'Demander')");
