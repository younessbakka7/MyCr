...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    if(array_key_exists('Del', $_POST)) 
    {
        if(isset($_POST["Rows"]))
        {
            if(!empty($_POST["Rows"]))
            {
                    ...
                    {
                        $Delete1 = $cn->prepare("DELETE from commande where Num_C = ?");
    ...
    if(array_key_exists('ADD', $_POST)) 
    {
                        ...
                        $Insert1 = $cn->prepare("INSERT INTO commande VALUES(NULL,?,?,?)");
                        $Insert1 -> execute(array($_POST["CCode"],$_POST["Cdate"],$_POST["Cetat"]));

                        echo "<script> alert('Bien Ajouter'); </script>";
    }
    if(array_key_exists('Save', $_POST))
    {
        $Update = $cn->prepare("UPDATE commande SET Code_Ut=? , Date_C=? , Etat_C=? WHERE Num_C = ?");
                        $Update -> execute(array($_POST["CCode2"],$_POST["Cdate2"],$_POST["Cetat2"],$_GET["ref"]));
...
{
    $SS = $_GET['search'];
$Voiture = $cn->prepare('SELECT * FROM commande c INNER JOIN utilisateur u on c.Code_Ut=u.Code_Ut WHERE c.Date_C =?');
$Voiture -> execute(array($_GET['search']));
if($Voiture->rowCount() != 0)
{
    ...
    for ($j=0; $j < $Voiture->rowCount() ; $j++) 
    {
        ...
        $Ref         = $lineV["Num_C"];
        $Full      = $lineV["Nom_Ut"]." ".$lineV["Prenom_Ut"];
        $Datee      = $lineV["Date_C"];
        $Etatt       = $lineV["Etat_C"];
...
if(!isset($_GET['row']) == true && !isset($_GET['search']) == true)
{
    $Voiture = $cn->prepare('SELECT * FROM commande c INNER JOIN utilisateur u on c.Code_Ut=u.Code_Ut');
$Voiture -> execute();
if($Voiture->rowCount() != 0)
{
    $Table = "";
    for ($j=0; $j < $Voiture->rowCount() ; $j++) 
    {
        ...
        <td>'.$Datee.'</td>
        <td>'.$Etatt.'</td>
        <td><a href="Commande.php?row='.$j+1 .'&ref='.$Ref.'"><button type="button" class=" btn btn-outline-info m-2">Update</button></a></td>
    </tr>';
    }
}

}
else if(isset($_GET['row']) == true)
{
...
/////////////////////////////////////       
    ...
    $Query = $cn->prepare("SELECT * from utilisateur");
$Query -> execute();
...
if($Query->rowCount() != 0)
{
    for ($j=0; $j < $Query->rowCount() ; $j++) 
    {
        ...
        $drop =$drop.'<option value="'.$lin["Code_Ut"].'">'.$lin["Nom_Ut"]." ".$lin["Prenom_Ut"].'</option>';
        $lin = $Query->fetch(PDO::FETCH_ASSOC);
    }
}
////////////////////////////////////
...
if($Voiture->rowCount() != 0)
{
    ...
    <tr class="text-white">
    <td><input class="form-check-input" type="checkbox" name="Rows[]" onclick="Checkone()" value="'.$Ref.'"></td>
    <th scope="col"><select class="form-select mb-3" name="CCode2" value="'.$Code_U.'" aria-label="Default select example" require>
    '.$drop.'
                                    </select></th>
    <th scope="col"><input class="form-control mb-3" type="date" name="Cdate2" value="'.$Datee.'" aria-label="default input example" require></th>
    
    <th scope="col"><select class="form-select mb-3"  name="Cetat2" aria-label="Default select example" require>
    ...
    <a href="Commande.php" ><button type="button" class="btn btn-outline-primary m-2">X</button></a>
    </th>
</tr>
</form>';   
       }
...
{
    $Table = '<span style="font-size:30px !important;color:red !important;">!!! Aucun Commande !!!</span>';
}
...
//}
else if(!isset($_GET['row']) == true && !isset($_GET['search']) == true)
{
...
else
{
    if($_GET["row"] != NULL && $_GET["ref"] != NULL)
   {
    ...
    <tr class="text-white">
    <td><input class="form-check-input" type="checkbox" name="Rows[]" onclick="Checkone()" value="'.$Ref.'"></td>
    <th scope="col"><select class="form-select mb-3" name="CCode2" aria-label="Default select example" require>
    '.$drop.'
                                    </select></th>
    <th scope="col"><input class="form-control mb-3" type="date" name="Cdate2" aria-label="default input example" require></th>
...
<head>
    <meta charset="utf-8">
    <title>Dashboard Commande</title>
    ...
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-secondary navbar-dark sticky-top px-4 py-0">
                ...
                <a href="Commande.php"><button type="button" class="btn btn-dark m-2">Clear</button></a>
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">Commande DashBoard</h6>
                    ...
                    <table class="table text-start align-middle table-bordered table-hover mb-0">
                            ...
                            <tfoot>
                                <tr class="text-white">
                                    ...
                                    <th scope="col"><select class="form-select mb-3" name="CCode" aria-label="Default select example" require>
                                        <?php 
                                        $Query = $cn->prepare("SELECT * from utilisateur");
                                        $Query -> execute();
                                        $lin = $Query->fetch(PDO::FETCH_ASSOC);
                                        if($Query->rowCount() != 0)
                                        {
                                            for ($j=0; $j < $Query->rowCount() ; $j++) 
                                            {
                                                ...
                                                echo '<option value="'.$lin["Code_Ut"].'">'.$lin["Nom_Ut"]." ".$lin["Prenom_Ut"].'</option>';
                                                $lin = $Query->fetch(PDO::FETCH_ASSOC);
                                            }
                                        }
                                        ?>
                                    </select></th>
                                    <th scope="col"><input class="form-control mb-3" type="date" name="Cdate" aria-label="default input example" require></th>
                                    
                                    <th scope="col"><select class="form-select mb-3" name="Cetat" aria-label="Default select example" require>
                                        <option value="en demande">en demande</option>
                                        <option value="Confirme">Confirme</option>
                                    </select></th>
