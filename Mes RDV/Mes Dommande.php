...
try {
  ...
  $Commande = $cn->prepare("SELECT * FROM `rdv` WHERE Code_Ut=?");
  ...
  if($Commande->rowCount() != 0)
{
  ...
  <p style="margin-top: -70px; padding-bottom:25px;" class="info full-name">RDV N°'.$Num.' </p>
          <div class="nn">
          <p class="cc info place">
                  <span style="font-size: 25px;">📝</span>'.$TYPE.'
                  
              </p>    
          
          <p style="" class="cc info place">
                  <span style="font-size: 25px; text-decoration: none;">📆 </span>'.$DATE.'
              </p>
          ...
          <a style="text-decoration:none;margin-top: -10px;" href="Detaille Dommande.php?num='.$NumR.'&res='.$_GET["res"].'" class="up action"> Detaills RDV </a>        
</div>';
...
<body>
    <form id="form1">
        <h1 class="tt">Mes Demandes</h1>
        <section class="bgg footer">
        <div class="share">
            <a href="../Dr.Car Market/My Actions.php?res=<?php echo $_GET["res"] ?>" class="fab las la-home"> </a>
        </div>
    </section>
        <div class="Contrats">
            <?php echo $Cart; ?>
